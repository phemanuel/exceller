...
$(function () {
    ...
    $(document).on('click', '#loadSummaryBtn', function () {
        ...
        $.get('/osce/results/summary', { date: selectedDate })
            .done(function (data) {
                summaryData = data;
    ...
    function buildTableHeader(stations) {
        let headerRow = `<th>Student Name</th><th>Department</th>`;
        stations.forEach(() => headerRow += `<th>Procedure/MCQ</th>`);
    ...
    function buildTableBody(students) {
        ...
        students.forEach(res => {
            let row = `
                <tr>
                    <td>${res.student.surname ?? ''} ${res.student.first_name ?? ''}<br>
                        <small>${res.student.admission_no ?? ''}</small></td>
            ...
            res.stations.forEach(st => {
                if (st.completed) {
                    row += `<td>
                        <small>
                            Station${stationCounter}: ${st.examiner_score}<br>
                            Station${stationCounter + 1}: ${st.mcq_score}<br>
            ...
            row += `<td><strong class="text-primary">${res.overall_total}</strong></td>
                    <td>${
                        res.overall_total > 0
...
$(document).on('click', '#exportExcelBtn', function () {
    ...
    summaryData.stations.forEach(() => headerRow.push('Procedure/MCQ'));
    ...
    summaryData.students.forEach(res => {
        ...
        res.stations.forEach(st => {
            if (st.completed) {
                row.push(`Station${stationCounter}: ${st.examiner_score} | Station${stationCounter + 1}: ${st.mcq_score} | Total: ${st.total_score}`);
            } else {
                row.push('—');
            }
            stationCounter += 2;
    ...
    // Get selected date and format it for the file name
    let selectedDate = $('#summaryDateInput').val();
    ...
    $(document).on('click', '#printSummaryBtn', function () {
        ...
        let printHtml = `<table border="1" cellspacing="0" cellpadding="5" width="100%"><thead><tr>
            <th>Student Name</th><th>Department</th>`;

        summaryData.stations.forEach(() => printHtml += `<th>Procedure/MCQ</th>`);
        ...
        summaryData.students.forEach(res => {
            ...
            printHtml += `<td><strong>${res.overall_total}</strong></td></tr>`;
        });
